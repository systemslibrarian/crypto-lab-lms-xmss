 ...
 *    files the node under `incomplete`. The hero, the reach bars and the
 *    Winternitz chain rows all paint gradient fills, so the text over them
 *    composites onto something axe will not measure.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. The footer here is an inline
 *    `opacity:.85` block wrapping every related-demo link, and its scripture
 *    line fades again to `.75` inside that. Note this lab collapses its
 *    animations to 0.01ms under reduced motion, which preserves end states —
 *    including the `.leaf.current` pulse, which loops indefinitely otherwise.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. `.leaf-grid` is a
 *    32-column, `max-height: 300px` scroller holding 1024 leaf buttons, so most
 *    of them sit outside its client box at any moment. Content scrolled past
 ...
 *    fall through to white, inventing a failure. Skip it, and rely on the leaves
 ...
 *    draws its Merkle tree, its Winternitz chains and its authentication-path
 *    climb out of plain divs and spans rather than SVG, and its only SVG is the
 *    header's `aria-hidden` hamburger icon — so this rule finds nothing here
 *    today. It is kept because it is cheap and silent when unused: an `<svg>`
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     *
     * This page renders 1024 leaf buttons in Exhibit 1, 34 Winternitz chain
     * rows in Exhibit 3 and a ten-level authentication-path climb in Exhibit 4,
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This lab has no <details>, but it does use the `hidden`
      // attribute on `#forge-reach`, which `checkVisibility()` catches for the
      // same reason; the gate scans its revealed state explicitly.
     ...
     * it, the `aria-hidden` decoration this lab carries — the OTS colour-swatch
     * legend, the `#auth-tree` diagram and the climb arrows — would be measured
     * as if each took its element's declared `color`, reporting ratios for
     * marks that carry no text at all.
     */
